...
// Player state to track performance
let linesCleared = 0;
let gameResets = 0;
...
function createPieceBasedOnPerformance() {
    const easyPieces = ["I", "O"];
    const hardPieces = ["S", "Z", "L", "J", "T"];

    // If the player has cleared many lines, give them harder pieces
    const piecePool = (linesCleared > 5) ? hardPieces : easyPieces;
    return createPiece(piecePool[Math.floor(Math.random() * piecePool.length)]);
}
